...
- **`backend/`**：基于 Python FastAPI 的后端服务器，处理 RAG 的核心逻辑。
  ...
  - 新增优化服务：`IndexBenchmarkService`, `QueryOptimizer`, `PostRetrievalOptimizer`。
  - 用于存储中间状态的本地文件管理系统（如 `01-loaded-docs`, `02-embedded-docs`, `07-benchmark-results` 等）。
...
**3. 向量库索引比较分析**（新增 `backend/services/index_benchmark_service.py`）

对同一数据集使用 **8 种 Chroma HNSW 索引变体**进行对比基准测试：

| 索引名称 | hnsw:space | M | ef_construction | 说明 |
|----------|------------|---|-----------------|------|
| `cosine_default` | cosine | 16 | 100 | 默认参数基线 |
| `cosine_high_recall` | cosine | 64 | 500 | 最高召回率 |
| `cosine_balanced` | cosine | 32 | 200 | 平衡型 |
| `cosine_fast` | cosine | 8 | 50 | 最快构建/检索 |
| `cosine_high_ef` | cosine | 16 | 500 | 高构建精度 |
| `cosine_high_m` | cosine | 64 | 100 | 高连接度 |
| `l2_default` | l2 | 16 | 100 | L2 距离基线 |
| `ip_default` | ip | 16 | 100 | 内积距离基线 |

**测量指标**：索引构建时间(s)、磁盘大小(MB)、平均/P50/P99 检索延迟(ms)、QPS

**新增 API**：
- `GET /index-presets` — 获取可用索引预设
- `POST /index-benchmark` — 执行索引对比
- `GET /benchmark-results` / `GET /benchmark-results/{id}` — 查看历史结果

**前端**：Indexing 页面新增"索引对比分析"Tab，支持多选索引类型、柱状图对比

**4. 检索前/后优化**（新增 `backend/services/query_optimizer.py` + `backend/services/post_retrieval.py`）

**检索前优化 (Pre-retrieval)**：

| 策略 | 说明 |
|------|------|
| Query Rewriting | 用 LLM 将模糊问题改写为精准检索查询 |
| Query Expansion | 生成多个查询变体提高召回率 |
| HyDE | 先生成假设性答案，用答案做向量检索 |
| Multi-Query | 将复杂问题拆解为多个子查询分别检索 |

**检索后优化 (Post-retrieval)**：

| 策略 | 说明 |
|------|------|
| Cross-Encoder Rerank | 用 Cross-Encoder 模型精确重排序 |
| MMR Diversity | 最大边际相关性，平衡相关性与多样性 |
| Deduplication | 基于语义相似度去除重复内容块 |
| Relevance Filter | 用 LLM 过滤不相关的检索结果 |
| Context Compression | 用 LLM 压缩提炼上下文 |

**新增 API**：
- `GET /optimization-options` — 获取可用优化策略
- `POST /search-optimized` — 带优化选项的增强搜索

**前端**：
- Search 页面新增"检索优化选项"折叠面板（前/后优化策略开关）
- Generation 页面新增"上下文优化选项"（压缩开关 + 最大上下文块数控制）
