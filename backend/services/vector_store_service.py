...
class VectorStoreService:
    ...
    def _index_to_milvus(self, embeddings_data: Dict[str, Any], config: VectorDBConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            # 创建索引 — 根据 config.index_mode 动态选择索引类型和参数
            index_params = client.prepare_index_params()

            milvus_index_type = self._get_milvus_index_type(config)
            milvus_index_params = self._get_milvus_index_params(config)
            logger.info(f"Creating Milvus index: type={milvus_index_type}, params={milvus_index_params}")
